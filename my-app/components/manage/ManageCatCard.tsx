    ...
    return (
      <div className="bg-white rounded-lg shadow-md overflow-hidden flex flex-col">
        {pet.primary_image_url ? (
          ...
          <div className="w-full h-64 bg-gray-100 flex items-center justify-center text-5xl text-gray-300">
            🐾
          </div>
        )}
        <div className="p-4 flex flex-col flex-1">
          <div className="flex items-start justify-between gap-2 mb-2">
            ...
            <span
              className={`text-xs px-2 py-1 rounded-full font-medium whitespace-nowrap ${STATUS_COLORS[pet.status]}`}>
              {pet.disposition_reason
                ? titleCase(pet.disposition_reason)
                : titleCase(pet.status)}
            </span>
          </div>
          <p className="text-sm text-gray-500 mb-1">
            {titleCase(pet.pet_type)}
            {pet.breed ? ` · ${pet.breed}` : ""}
          </p>
          <p className="text-sm text-gray-500 mb-1">
            {titleCase(pet.sex)}
          ...
          {showRemoveForm ? (
            <form onSubmit={handleRemove} className="mt-auto">
              ...
              <p className="text-sm font-medium text-gray-700 mb-2">
                Record disposition for {pet.name}
              </p>
      ...
      <form onSubmit={handleUpdate}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <select
              name="intake_reason"
              defaultValue={pet.intake_reason}
              className={inputCls}>
            ...
            {spayedNeutered && (
              <div>
                ...
                <input
                  ...
                  type="date"
                  defaultValue={pet.spay_neuter_date ?? ""}
                  className={inputCls}
                />
              </div>
            )}
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              name="disposition_date"
              type="date"
              defaultValue={pet.disposition_date ?? ""}
          ...
          {hasDisposition && (
            <div>
              ...
              <select
                name="disposition_reason"
                defaultValue={pet.disposition_reason ?? ""}
                className={inputCls}>
