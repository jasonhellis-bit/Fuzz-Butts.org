...
import { setFeatured, endFeatured } from "@/actions/featuredActions";
        ...
        {current ? (
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {current.pet_breed && (
                <p className="text-sm text-gray-500">{current.pet_breed}</p>
              )}
              <p className="text-xs text-gray-400 mt-1">
                Since {formatDate(current.start_date)}
                {current.end_date
                  ? ` · Until ${formatDate(current.end_date)}`
                  : " · Open-ended"}
              </p>
            </div>
            {confirmEnd ? (
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        ...
        {pets.length === 0 ? (
          <p className="text-sm text-gray-500">
            No pets available to feature. Add pets with &quot;available&quot; or
            &quot;pending adoption&quot; status first.
          </p>
        ) : (
              ...
              <select name="pet_id" required className={inputCls}>
                ...
                {pets.map((p) => (
                  <option key={p.id} value={p.id}>
                    {p.name}
                    {p.breed ? ` · ${p.breed}` : ""} ({titleCase(p.pet_type)})
                  </option>
                ))}
              </select>
            </div>
