...
export async function updateUser(
  ...
  const phone = digits
    ? `+1${digits.startsWith("1") && digits.length === 11 ? digits.slice(1) : digits}`
    : "";
  ...
  const [{ data: existing }, { data: existingAuth }] = await Promise.all([
    adminClient
      .from("users")
      .select("first_name, last_name, title, roles, status")
      .eq("id", id)
      .single(),
  ...
  if (existing?.title !== title) changes.push("title updated");
  if (existing?.roles !== roles)
    changes.push(`roles changed to: ${roles || "none"}`);
  if (existing?.status !== status) changes.push(`status changed to: ${status}`);
  if (existingAuth.user?.email !== email) changes.push("email updated");
  if (existingAuth.user?.phone !== phone) changes.push("phone updated");
