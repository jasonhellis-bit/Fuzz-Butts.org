...
export async function updatePet(formData: FormData): Promise<{ error?: string }> {
  ...
  if (image && image.size > 0) {
    ...
    if (existingImages && existingImages.length > 0) {
      await adminClient.storage.from("pet-images").remove(existingImages.map((i) => i.storage_path));
...
export async function removePet(formData: FormData): Promise<{ error?: string }> {
  const adminClient = createAdminClient();
  const userId = await getCurrentUserId();
...
export async function getPetImages(petId: string): Promise<PetImageData[]> {
  ...
  return (data ?? []).map((img) => ({
    ...img,
    url: adminClient.storage.from("pet-images").getPublicUrl(img.storage_path).data.publicUrl,
  }));
}

export async function uploadPetImage(petId: string, imageFile: File): Promise<{ error?: string }> {
  const adminClient = createAdminClient();
  const userId = await getCurrentUserId();
...
export async function setPrimaryImage(petId: string, imageId: string): Promise<{ error?: string }> {
  const adminClient = createAdminClient();
  const userId = await getCurrentUserId();
...
export async function deleteImageById(petId: string, imageId: string, storagePath: string): Promise<{ error?: string }> {
  const adminClient = createAdminClient();
  const userId = await getCurrentUserId();
